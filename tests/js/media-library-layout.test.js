...
test("music and video playlist column widths persist under separate Settings keys", async () => {
  ...
  initLayout(musicCtx);
  musicCtx.layoutApi.applyPlaylistColumnWidths({filename: 200, path: 300, reorder: 56}, true);
  ...
  initLayout(videoCtx);
  videoCtx.layoutApi.applyPlaylistColumnWidths({filename: 120, path: 400, reorder: 56}, true);
...
test("refreshPlaylistColumnWidths refits fixed px columns after the list width changes", async () => {
  ...
  initLayout(ctx);
  ctx.layoutApi.applyPlaylistColumnWidths({filename: 220, path: 340, reorder: 56}, false);
  ...
  assert.ok(parts[0] + parts[1] + parts[2] > 300);
});
