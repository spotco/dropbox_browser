...
class WebUiTests(AppTestCase):
    ...
    def test_sync_controls_render_in_separate_view_and_sync_columns(self) -> None:
        ...
        self.assertIn("var loadButtonDefaultText = loadButton.textContent || 'Load Current Folder';", js)
        self.assertIn("var defaultPollDelayMs = 4000", js)
        self.assertNotIn("var maxPollDelayMs", js)
        self.assertNotIn("pollDelayMs", js)
        self.assertIn("var loadTimer = null;", js)
        self.assertIn("var libraryPollingActive = false;", js)
        self.assertIn("var lastLibraryPollResponseAt = 0;", js)
        self.assertIn("var libraryPollSequence = 0;", js)
        self.assertIn("var musicPaneWidthSettingKey = 'music-pane-widths';", js)
        self.assertIn("var defaultMusicPanePercents = [35, 38.333333, 26.666667];", js)
        self.assertIn("fetch(libraryUrl(isRefresh, scheduledDelayMs))", js)
        self.assertIn("'/music/endpoints/library?path=' + encodeURIComponent(libraryRoot)", js)
        self.assertIn("'&poll_seq=' + encodeURIComponent(String(libraryPollSequence))", js)
        self.assertIn("'&poll_delay_ms=' + encodeURIComponent(String(scheduledDelayMs || 0))", js)
        self.assertIn("function normalizeMusicPanePercents(values)", js)
        self.assertIn("function musicPaneResizeEnabled()", js)
        self.assertIn("function applyMusicPanePercents(widths, persist)", js)
        self.assertIn("function readSavedMusicPanePercents()", js)
        self.assertIn("function startMusicPaneResize(resizerIndex, ev)", js)
        self.assertIn("Settings.get(musicPaneWidthSettingKey, defaultMusicPanePercents)", js)
        self.assertIn("Settings.set(musicPaneWidthSettingKey, currentMusicPanePercents);", js)
        ...
        self.assertIn("function updateLoadButtonTimer()", js)
        self.assertIn("loadButton.textContent = loadButtonDefaultText + ' (' + elapsedSeconds + ')';", js)
        self.assertIn("function startLibraryPollingUi()", js)
        self.assertIn("function stopLibraryPollingUi()", js)
        self.assertIn("function libraryPollingMessage(data, previousSnapshot)", js)
        ...
        self.assertIn("if (!libraryRequested || !playbackUiMayPaint()) return;", js)
        self.assertIn("function shouldPollLibrary()", js)
        self.assertIn("if (!shouldPollLibrary()) return;", js)
        ...
        self.assertIn("if (ev.detail.mode === 'music-player') {", js)
        self.assertIn("else {", js)
        self.assertIn("clearPlaybackUiPaintTimer();", js)
        ...
        self.assertIn("function paintPlaylist()", js)
        self.assertIn("var playlistRenderDirty = false;", js)
        self.assertIn("var playlistSelectionDirty = false;", js)
        self.assertIn("var pendingPlaylistFocusRemotePath = null;", js)
        self.assertIn("pendingPlaylistFocusRemotePath = remotePath;", js)
        ...
        self.assertIn("libraryPlaylistResizer.addEventListener('pointerdown'", js)
        self.assertIn("playlistPlaybackResizer.addEventListener('pointerdown'", js)
        ...
        self.assertIn("var playbackUiThrottleMs = 1000;", js)
        self.assertIn("var libraryRenderDirty = false;", js)
        self.assertIn("var pendingLibraryStatusText = null;", js)
        self.assertIn("function playbackUiMayPaint()", js)
        self.assertIn("return !document.hidden && isVisible();", js)
        self.assertIn("if (document.hasFocus()) {", js)
        self.assertIn("playbackUiPaintTimer = window.setTimeout(function () {", js)
        self.assertIn("function repaintPlaybackDisplay()", js)
        self.assertIn("function flushDeferredMusicPaneUpdates()", js)
        self.assertIn("function resumeLibraryUpdates()", js)
        self.assertIn("document.addEventListener('visibilitychange', function () {", js)
        self.assertIn("window.addEventListener('focus', function () {", js)
        self.assertIn("flushDeferredMusicPaneUpdates();", js)
        self.assertIn("clearPlaybackUiPaintTimer();", js)
        ...
        self.assertIn("window.addEventListener('resize', function () {", js)
        self.assertIn("applyMusicPanePercents(currentMusicPanePercents, false);", js)
        self.assertIn("applyMusicPanePercents(readSavedMusicPanePercents(), false);", js)
