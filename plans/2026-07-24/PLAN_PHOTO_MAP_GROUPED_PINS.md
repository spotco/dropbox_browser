...
- [ ] Add a `Grouping distance` control beside Date range in the existing
  toolbar-controls region.  Offer Off/0 m, 5 m, 10 m, 20 m, 50 m, and 100 m;
  select 20 m by default.
- [ ] Keep future controls in the existing extensible controls region and keep
...
- [ ] Recompute map marker data when the control changes, without rereading
  the remote listing or metadata ranges.  Reuse the current generation's
  metadata and thumbnail result cache.
- [ ] Add focused host/config tests for the default, Off option, numeric values,
  and re-grouping without a new metadata request.

### 2. Build deterministic browser-side photo groups

- [ ] Add a pure grouping helper that accepts located photo items and a meter
  ...
  O(n²) scan.
- [ ] Ensure grouping does not create transitive chains that extend far beyond
  the selected radius.  Use deterministic input ordering and keep every
  member within the group's representative/centroid radius; document the
  representative rule in the helper.
- [ ] Preserve each member's source path, metadata, and existing thumbnail
...
- [ ] Leave videos out of groups and keep them as their current individual
  marker records.
- [ ] Add tests for singleton behavior, radius boundaries, stable ordering,
  multiple separated groups, large-group counts, and photo/video separation.

### 3. Render emphasized grouped pins without spiderfy

- [ ] Extend map marker reconciliation to accept grouped records without
...
- [ ] Keep the group center visibly connected to the geographic location and
  preserve keyboard focus, title/accessible label, click behavior, and normal
  singleton photo/video visuals.
- [ ] Configure the MarkerClusterGroup so max-zoom clicks never spiderfy.
...
- [ ] Clicking a grid thumbnail selects that member's details in the popup:
...
- [ ] Add tests for grouped popup HTML, count/grid ordering, loading/error
...
- [ ] Make visible grouped pins request member thumbnails only while the group
  marker is individually visible.  A clustered/hidden grouped pin must not
  schedule all of its members.
- [ ] On distance changes, map moves, date changes, pane switches, and folder
  changes, abort stale grouped-member requests without deleting successful
  in-memory thumbnail results or allowing late results to mutate a newer
  generation.
- [ ] Keep individual-photo thumbnail behavior unchanged when grouping is Off
  or no nearby partner exists.
- [ ] Extend diagnostics with group count, grouped-member count, grouping
