...
- [x] Inspect the existing Playwright harness, music-player UI flow, and test
  support helpers before editing.
- [x] Add a dedicated in-process integration server harness with no subprocess
  rclone adapter.
- [x] Add one committed deep-music fixture file for this test.
- [x] Add narrow test-only polling/timing overrides needed for practical
  runtime.
- [x] Add one Playwright integration test for the requested flow.
- [x] Update the relevant docs.
- [ ] Run targeted verification.

## Step 1 - Confirm Existing Coverage And Harness Boundaries

- [x] Re-read the existing music-library endpoint tests in
  `tests/test_music_endpoints.py`.
- [x] Re-read the existing folder-cache worker tests in
  `tests/test_folder_info_workers.py`.
- [x] Re-read the current Playwright harness in:
  - `tests/e2e/support/server.js`
  - `tests/e2e/support/run_server.py`
- [x] Confirm the current Playwright harness shells out through
  `tests/fake_rclone.cmd` / `tests/fake_rclone.py`.
- [x] Preserve the existing harness for current E2E tests; do not break the
  current `smoke.spec.js` flow.
- [x] Keep this new integration test isolated to the explicitly requested music
  scenario only.
...
- [x] Add a dedicated Python integration server entrypoint under
  `tests/e2e/support/` for the music integration test.
- [x] Reuse the existing fake-rclone E2E infrastructure where practical:
  - `tests/fake_rclone.cmd`
  - `tests/fake_rclone.py`
  - the existing stdlib HTTP server stack
- [x] If the existing fake-rclone fixture format cannot express the staged
...
- [x] Use isolated temp/cache paths for the integration run.
- [x] Keep the integration server on a non-default port, using Playwright port
  configuration such as `8011`.
- [x] Make the harness expose any artifacts the Playwright test needs for
  backend assertions:
  ...
  - a way to confirm the fake-rclone test path was used and not the real
    `rclone.exe`.
- [x] Keep the integration harness specific and narrow; do not replace the
...
- [x] Create one fixture file under `tests/e2e/fixtures/`.
- [x] Keep the fixture audio-only.
- [x] Base the structure on the current observed cached `dropbox:music` tree
...
- [x] Make the root `/?path=music` page load immediately from simulated direct
  listing data.
- [x] Ensure descendant folder-cache metadata for some subtrees is not complete
  when the page first renders.
- [x] Add at least one intentionally delayed subtree that starts as incomplete
  so Song Library can show a visible `not cached` row initially.
- [x] Add staged delayed completion for at least two checkpoints when practical:
  - initial partial state;
  - intermediate partial growth;
  - final completed state.
- [x] Drive staged completion with explicit gates or deterministic waits inside
  the fake-rclone fixture/harness or an equivalent narrow test helper.
- [x] Do not let the request thread block waiting for descendant completion.
...
- [x] Add a narrow test-only override for the music-library poll interval.
- [x] Keep the production default unchanged.
- [x] Set the integration test override to a practical fast interval such as
  `100-200 ms`.
- [x] Keep the test runtime target roughly within `3-8 s`.
- [x] Ensure the integration test fails by timeout at `15 s`.
- [x] Prefer a server-rendered or app-configured override path over a global
  production JS constant change.
- [x] Keep the override implementation explicit enough that future tests can
...
- [x] Start from `/?path=music`.
- [x] Switch the bottom pane to `Music Player`.
- [x] Click `Load Current Folder`.
- [x] Measure and assert that the initial load action returns quickly, with a
  concrete threshold such as under `250 ms`.
- [x] Assert the initial Song Library state is partial and user-visible:
  ...
  - exact initial visible folder/song counts match the expected staged fixture
    state.
- [x] Assert that at least one relevant file-browser folder row is still
  incomplete at this stage.

### Intermediate Assertions

- [x] Release or wait for the first delayed subtree completion.
- [x] Assert the next music-library poll updates the browser without another
  manual click.
- [x] Assert exact intermediate folder/song counts.
- [x] Assert newly available songs appear in Song Library after the matching
  file-browser folder is no longer loading.
- [x] If the harness supports a second staged subtree cleanly, repeat once more
  for another exact-count intermediate checkpoint.

### Final Assertions

- [x] Wait for all delayed subtree work and background folder-cache workers to
  complete.
- [x] Assert the final music-library state is complete in the browser.
- [x] Assert the browser has received and rendered the final completed poll.
- [x] Assert exact final Song Library counts:
  - final folder count;
  - final song count.
- [x] Assert representative expected deep rendered paths are present.
...
- [x] Use explicit staged completion gates or deterministic checkpoint waits.
- [x] Record exact expected counts for:
  - initial partial response;
  - each intermediate staged response used by the test;
  - final completed response.
- [x] Avoid brittle assertions of the form:
...
- [x] Update `docs/testing.md` to describe:
...
- [x] Update `docs/background-workers.md` to describe the integration-test
...
- [x] Keep the docs concise and implementation-specific.
...
- [x] Run the specific new Playwright test directly.
