...
(function () {
  ...
  function reportCompatibilitySeekTiming(milestone, fields) {
    var snapshotTarget = fields
      ...
      : undefined;
    var details = Object.assign({}, currentProcessedRangeSnapshot(snapshotTarget), fields || {});
  ...
  function compatibilitySeekableRanges() {
    ...
    return mediaRangesSummary(ctx.els.videoEl.seekable);
  }
  ...
  function attachCompatibilityVideo(playlistUrl, title, meta, startSeconds, surfaceSyncToken) {
    ...
    reportCompatibilitySeekTiming('hls_attach_started', {
    ...
    if (Hls && typeof Hls.isSupported === 'function' && Hls.isSupported()) {
      ...
      ctx.state.hlsController.on(Hls.Events.MANIFEST_LOADED, function (_eventName, data) {
        ...
        ctx.state.compatibilityManifestStallTimer = window.setTimeout(function () {
          ...
          reportCompatibilitySeekTiming('hls_manifest_parse_stall', {
            stall_ms: 5000,
  ...
  function trySeekCompatibilityInSession(active, probePayload, clampedTarget, reason, phase) {
    ...
    var decision = compatibilityInSessionSeekDecision({
      ...
      selectedAudioStreamIndex: selectedAudioStreamIndex(active, probePayload),
    ...
    reportCompatibilitySeekTiming('compatibility_seek_evaluated', {
      seek_reason: reason || '',
    ...
    reportPlaybackTiming('in_session_seek_requested', {
    ...
    setPlaybackSummary(
      ...
      'Playing through a local HLS compatibility session from '
        + formatPlaybackTime(ctx.state.compatibilityStartSeconds) + '.'
    );
    ...
    if (wasPlaying) requestVideoPlay();
    reportPlaybackTiming('in_session_seek_complete', {
      requested_time: clampedTarget,
      seek_reason: reason || '',
    });
    return true;
  ...
  async function restartCompatibilityAt(targetSeconds, reason) {
    ...
    reportCompatibilitySeekTiming('restart_seek_started', {
    ...
    if (Number.isFinite(duration) && duration > 0) {
      ...
      if (clampedTarget !== preClampTarget || rawTargetSeconds !== clampedTarget) {
        reportCompatibilitySeekTiming('restart_target_clamped', {
          raw_target_seconds: rawTargetSeconds,
    ...
    var probePayload = cachedProbePayload || await ensureAudioTracksForItem(active);
    ...
    try {
      ...
      reportCompatibilitySeekTiming('session_create_response', {
        requested_time: clampedTarget,
  ...
  if (ctx.els.progressSliderEl) {
    ...
    ctx.els.progressSliderEl.addEventListener('change', function () {
      ...
      if (Number.isFinite(nextTime) && nextTime >= 0) {
        ...
        revealControlsOverlay();
        if (ctx.state.seekRestartInProgress) {
          reportCompatibilitySeekTiming('seek_queued_during_restart', {
        ...
        reportCompatibilitySeekTiming('scrub_restart_requested', {
